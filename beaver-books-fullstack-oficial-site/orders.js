...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    for (const item of itens) {
      ...
      subtotal += parseFloat(rows[0].preco) * item.quantidade;
    }
    ...
    for (const item of itens) {
      const { rows } = await client.query('SELECT preco FROM books WHERE id=$1', [item.livro_id]);
      await client.query(
        `INSERT INTO order_items (order_id, livro_id, quantidade, preco_unitario)
         VALUES ($1,$2,$3,$4)`,
        [pedidoId, item.livro_id, item.quantidade, rows[0].preco]
      );
      await client.query(
  ...
  try {
    ...
    const itens = await pool.query(
      `SELECT oi.*, b.titulo, b.autor FROM order_items oi
       JOIN books b ON b.id = oi.livro_id WHERE oi.order_id=$1`,
      [req.params.id]
    );
